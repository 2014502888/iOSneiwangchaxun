...
final class QueryEngine: ObservableObject {
    ...
    @Published private(set) var results: [MailResult] = []
    ...
    var abnormalResults: [MailResult] {
        results.filter(\.isAbnormal)
    }

    var failedResults: [MailResult] {
    ...
    var duplicateResults: [MailResult] {
    ...
    private func run(nums: [String], start: Date) async {
        ...
        await withTaskGroup(of: (index: Int, result: MailResult).self) { group in
            for (index, num) in nums.enumerated() {
                ...
                group.addTask {
                    ...
                    outcome = await TrackAPI.fetch(num)
                    var retryCount = 0
                    ...
                    case .success(let raws):
                        result.traces = raws.map(TrackParsing.traceNode)
                        ...
                        if let first = raws.first, let last = raws.last {
                            ...
                            let times = raws.compactMap { trace -> Date? in
                                if let t = trace.operatingTime, let d = TrackParsing.parseDate(t) { return d }
                                if let t = trace.acceptTime, let d = TrackParsing.parseDate(t) { return d }
                                return nil
                            }
    ...
    private static func markDuplicates(_ list: [MailResult]) -> [MailResult] {
        var seen = Set<String>()
        var out: [MailResult] = []
        for r in list {
...
private func calculateDurationToDeliver(raws: [MailTraceRaw]) -> String {
    ...
    for raw in raws {
        if let t = raw.operatingTime, let d = TrackParsing.parseDate(t) {
            firstDate = d
            break
        }
        if let t = raw.acceptTime, let d = TrackParsing.parseDate(t) {
            firstDate = d
            break
        }
    }
